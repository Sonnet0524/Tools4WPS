...
breadcrumb: WPS365应用开发 > 通讯录 > 事件 > 用户状态变更
source: raw_md/app-integration-dev/wps365/server/address-book/event/user-status-change.md
---
...
| ∟ status   | string        | 否        | 用户状态（变更前）<br/>`notactive`：未激活；`active`：已激活；`disabled`：已禁用；`dimission`：已离职 |
