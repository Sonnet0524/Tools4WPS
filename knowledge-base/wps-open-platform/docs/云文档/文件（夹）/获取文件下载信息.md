...
| <div style="white-space: nowrap;">storage_base_domain</div> | `string` | 否 | wps.cn/kdocs.cn/wps365.com(国际化),签发的存储网关地址会根据base_domain优先匹配 \*.wps.cn/\*.kdocs.cn/\*.wps365.com。<br>            为空会根据请求的host优先选择关联的base_domain.(注意目前有些旧文件未被存储网关托管,针对这些文件的下载此参数可能失效) | - 
