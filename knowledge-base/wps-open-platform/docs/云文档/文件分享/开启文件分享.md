...
| **<div style="white-space: nowrap;">权限要求</div>** | <div><div>查询和管理文件分享(应用授权) `kso.file_link.readwrite`</div><div style="margin-top: 5px;"></div><div>查询和管理文件分享(用户授权) `kso.file_link.readwrite`</div><div style="margin-top: 5px;"></div><div>MCP开放能力(用户授权) `kso.mcp.readwrite`</div></div> |
